...
- **`.ab-chip` is dead on `<button>` above 820px.** `.ab-root button`
  (`globals.css:208`, specificity 0,1,1) resets `font` and `border` and outranks
  `.ab-chip` (0,1,0), so the desktop language toggle renders as plain 16px
  borderless text and always has. 012 restored the component **inside the
  `<= 820px` media query only**, by owner's call, so the mobile cluster is
  coherent and the desktop nav is byte-identical. Fixing it globally is a
  one-line specificity change and a **visible desktop restyle** — owner's call.
