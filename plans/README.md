...
- **The 404 island uses `aria-pressed`, the nav now uses `aria-current`.**
  `_not-found-controls.tsx` has its own two-button language control predating
  this work. Both are valid for a two-option switch, but one site should pick
  one. Aligning the 404 is a two-line change nobody has claimed.
